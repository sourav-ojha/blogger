...
router.get("/blog", async (req, res) => {
  ...
  try {
    const posts = await Post.find({ is_published: true }).select({
      _id: 0,
      __v: 0,
    });
...
router.get("/blog/:post_id/", async (req, res) => {
  ...
  if (post_id) {
    try {
      ...
      const username = token == null ? decodeJWT(token) : null; //token type is string
      if (!username) {
...
router.post("/blog", auth, async (req, res) => {
  let { title, content, keywords, category, cover_img, is_published } =
    sanitize(req.body); // Sanitize the data to prevent injection attacks
  ...
  if (title && content && keywords && category && cover_img) {
    ...
    return res.status(400).json({
      msg: "Please provide a title, content, category, is_published and keywords!",
    });
  }
});
...
router.patch("/blog/:post_id", auth, async (req, res) => {
  let post_id = sanitize(req.params.post_id);
  let { title, content, keywords, category, is_published, cover_img } =
    sanitize(req.body);
  ...
  if (post_id && title && content && keywords && category) {
    ...
    return res
      .status(400)
      .json({ msg: "Please provide a title, content, category and keywords!" });
  }
});
...
router.delete("/blog/:post_id", auth, async (req, res) => {
  ...
  if (post_id) {
    try {
      ...
      let actual_owner_of_post = await Post.findOne({
        username: decodeJWT(req.header("Authorization").split(" ")[1]),
        post_id: post_id,
      });
  ...
  if (search_query) {
    try {
      ...
      let posts = await Post.find({
        $or: [
          { title: RegExp(search_query, "i") },
          { keywords: RegExp(search_query, "i") },
        ],
        is_published: true,
      }).select({ _id: 0, __v: 0 });
      if (posts.length > 0) {
        ...
        return res
          .status(200)
          .json({ msg: "No result found for " + search_query });
      }
    } catch (err) {
      return res.status(500).json({ msg: "Something went wrong!" });
    }
...
router.post("/blog/:post_id/like", auth, async (req, res) => {
  ...
  if (post_id) {
    try {
      ...
      let user_liked_post = await Likes.findOne({
        post_id: post_id,
        username: decodeJWT(req.header("Authorization").split(" ")[1]),
      });
      if (user_liked_post) {
